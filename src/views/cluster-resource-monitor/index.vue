<template>
  <div class="input-row">
    <div class="input-container">
      ...
      <input v-model="memoryUsage" class="large-input" />
    </div>

    <div class="input-container">
      <label class="large-font">cpu利用率(%):</label>
      <input v-model="CpuUsage" class="large-input" />
    </div>

    <div class="input-container">
      <label class="large-font">磁盘利用率(%):</label>
      <input v-model="diskUsage" class="large-input" />
    </div>
  </div>
  ...
  <div class="input-row">
    <div class="input-container">
      ...
      <input v-model="memSize" class="large-input" />
    </div>

    <div class="input-container">
      <label class="large-font">cpu总核数:</label>
      <input v-model="cpuNum" class="large-input" />
    </div>

    <div class="input-container">
      <label class="large-font">store进程健康状态:</label>
      <input v-model="StoreStaus" class="large-input" />
    </div>
  </div>
...
export default {
  ...
  methods: {
    async fetchStoreStatus() {
      ...
      // 每隔一秒重新获取数据，实现实时更新
      setTimeout(this.fetchStoreStatus, 1000) // 1000毫秒（1秒）更新一次
    },
    async fetchCpuNum() {
      try {
        const response = await axios.get(
